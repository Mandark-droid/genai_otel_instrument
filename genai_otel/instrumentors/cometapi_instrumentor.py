...
class CometAPIInstrumentor(BaseInstrumentor):
    ...
    def _instrument_openai_client(self, client):
        ...
        # CometAPI exposes embeddings through the same OpenAI-compatible
        # interface as chat. Without this, embedding calls made through a
        # CometAPI client produced no span at all: the generic OpenAI
        # instrumentor's own embeddings wrapping never fires for it either -
        # find_base_url_claim() skips it in favor of this dedicated
        # instrumentor. (Anthropic has no embeddings API, so
        # _instrument_anthropic_client needs no equivalent.)
    ...
    def _extract_embedding_attributes(
        ...
        """Extract attributes from a CometAPI embeddings call (OpenAI-SDK path)."""
        model = kwargs.get("model", "unknown")
    ...
    def _extract_response_attributes(self, result) -> Dict[str, Any]:
        ...
        # Embeddings responses (OpenAI-SDK path only) carry vectors instead
        # of choices.
    ...
    def _add_content_events(self, span, result, request_kwargs: dict):
        ...
        # Embeddings requests (OpenAI-SDK path) carry `input` rather than
        # `messages`.
