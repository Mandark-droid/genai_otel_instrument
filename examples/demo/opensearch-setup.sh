...
# Version 2.0 - Now includes Evaluation Metrics (PII, Toxicity, Bias, Prompt Injection, Restricted Topics, Hallucination)
...
echo "Creating genai-ingest-pipeline with evaluation metrics support..."
curl -X PUT $CURL_AUTH "$OPENSEARCH_URL/_ingest/pipeline/genai-ingest-pipeline" \
  -H 'Content-Type: application/json' \
  -d '{
  "description": "GenAI OTel Instrumentation Pipeline v2.0 - Extracts GenAI semantic convention fields and evaluation metrics (PII, toxicity, bias, prompt injection, hallucination) from Jaeger span tags.",
  "version": 2,
  "processors": [
    {
      "script": {
...
echo "Pipeline created: genai-ingest-pipeline (v2.0 with evaluation metrics)"
echo "Index template created: genai-jaeger-span-template"
echo ""
echo "New evaluation metrics fields:"
echo "  - PII Detection (prompt & response)"
echo "  - Toxicity Detection (prompt & response)"
echo "  - Bias Detection (prompt & response)"
echo "  - Prompt Injection Detection"
echo "  - Restricted Topics (prompt & response)"
echo "  - Hallucination Detection (response)"
echo ""
