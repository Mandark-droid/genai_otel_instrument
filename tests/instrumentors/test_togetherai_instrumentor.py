...
class TestTogetherAIInstrumentor(unittest.TestCase):
    ...
    def test_extract_usage_exception_handling(self):
        ...
        # Test with response that raises exception when accessing usage
        mock_response = MagicMock()
        mock_response.usage = property(lambda self: 1 / 0)  # Raises ZeroDivisionError

        result = instrumentor._extract_usage(mock_response)
        self.assertIsNone(result)

    def test_instrument_with_newer_together_sdk(self):
        """Test instrumentation with newer Together SDK (client-based API)."""
        # Create mock together module with Together class
        mock_together = MagicMock()
        mock_together.Together = MagicMock
        ...
        with patch.dict("sys.modules", {"together": mock_together}):
            ...
            # Verify Together.__init__ was wrapped
            self.assertIsNotNone(mock_together.Together.__init__)
            self.assertTrue(hasattr(mock_together.Together, "__init__"))

    def test_instrument_client_chat_completions(self):
        """Test _instrument_client wraps chat completions."""
        instrumentor = TogetherAIInstrumentor()

        # Create mock client with chat.completions.create
        mock_client = MagicMock()
        original_create = MagicMock(return_value="chat response")
        mock_client.chat.completions.create = original_create

        # Mock the instrumentor's methods
        instrumentor.tracer = MagicMock()
        instrumentor.request_counter = MagicMock()
        instrumentor.token_counter = MagicMock()
        instrumentor.latency_histogram = MagicMock()
        ...
        # Verify the create method was wrapped
        self.assertIsNotNone(mock_client.chat.completions.create)
        self.assertTrue(callable(mock_client.chat.completions.create))

    def test_extract_chat_attributes(self):
        """Test _extract_chat_attributes extracts correct attributes."""
        instrumentor = TogetherAIInstrumentor()
        ...
        attrs = instrumentor._extract_chat_attributes(None, [], kwargs)

        self.assertEqual(attrs["gen_ai.system"], "together")
        self.assertEqual(attrs["gen_ai.request.model"], "togethercomputer/llama-2-7b-chat")
        ...
        self.assertEqual(attrs["gen_ai.request.max_tokens"], 150)

    def test_extract_chat_attributes_minimal(self):
        """Test _extract_chat_attributes with minimal parameters."""
        instrumentor = TogetherAIInstrumentor()
        ...
        self.assertNotIn("gen_ai.request.temperature", attrs)
        self.assertNotIn("gen_ai.request.top_p", attrs)
        self.assertNotIn("gen_ai.request.max_tokens", attrs)

    def test_extract_complete_attributes(self):
        """Test _extract_complete_attributes extracts correct attributes."""
        instrumentor = TogetherAIInstrumentor()
        ...
        attrs = instrumentor._extract_complete_attributes(None, [], kwargs)

        self.assertEqual(attrs["gen_ai.system"], "together")
        self.assertEqual(attrs["gen_ai.request.model"], "togethercomputer/GPT-NeoXT-Chat-Base-20B")
        self.assertEqual(attrs["gen_ai.operation.name"], "complete")

    def test_instrument_exception_with_fail_on_error_true(self):
        """Test instrumentation exception handling when fail_on_error is True."""
        mock_together = MagicMock()
        mock_together.Together = property(lambda self: 1 / 0)  # Raises exception
    ...
    def test_instrument_exception_with_fail_on_error_false(self):
        """Test instrumentation exception handling when fail_on_error is False."""
        mock_together = MagicMock()
        mock_together.Together = property(lambda self: 1 / 0)  # Raises exception
