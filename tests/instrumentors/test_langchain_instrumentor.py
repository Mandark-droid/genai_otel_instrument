...
class TestLangChainInstrumentor(unittest.TestCase):
    ...
    def test_wrapped_chain_call(self):
        ...
        with patch.dict(
            ...
            self.assertEqual(attribute_dict["langchain.chain.type"], "MockChain")
            self.assertIn("session.id", attribute_dict)
            self.assertIn("langchain.session.id", attribute_dict)
    ...
    def test_wrapped_agent_call(self):
        ...
        with patch.dict(
            ...
            calls = mock_span.set_attribute.call_args_list
            attribute_dict = {call[0][0]: call[0][1] for call in calls}
            self.assertEqual(attribute_dict["langchain.agent.name"], "test_agent")
            self.assertIn("session.id", attribute_dict)
            self.assertIn("langchain.session.id", attribute_dict)
    ...
    def test_resolve_session_id_from_kwargs(self):
        """Test session ID resolution from kwargs."""
        instrumentor = LangChainInstrumentor()
        instrumentor.config = None

        session_id = instrumentor._resolve_session_id(None, (), {"session_id": "kwarg-session-123"})
        self.assertEqual(session_id, "kwarg-session-123")

    def test_resolve_session_id_from_input_dict(self):
        """Test session ID resolution from first positional arg dict."""
        instrumentor = LangChainInstrumentor()
        instrumentor.config = None

        session_id = instrumentor._resolve_session_id(
            None, ({"session_id": "input-session-456"},), {}
        )
        self.assertEqual(session_id, "input-session-456")

    def test_resolve_session_id_from_extractor(self):
        """Test session ID resolution from OTelConfig extractor."""
        instrumentor = LangChainInstrumentor()
        instrumentor.config = MagicMock()
        instrumentor.config.session_id_extractor = lambda inst, a, k: "extractor-session"

        session_id = instrumentor._resolve_session_id(None, (), {})
        self.assertEqual(session_id, "extractor-session")

    def test_resolve_session_id_autogenerated(self):
        """Test that UUID is auto-generated when no other source exists."""
        import uuid
    ...
    def test_resolve_session_id_priority_order(self):
        """Test that kwargs session_id takes priority over input dict."""
        instrumentor = LangChainInstrumentor()
        instrumentor.config = None
        ...
        session_id = instrumentor._resolve_session_id(
            ...
            {"session_id": "from-kwargs"},
        )
        self.assertEqual(session_id, "from-kwargs")

    def test_chat_model_invoke_has_session_id(self):
        """Test that chat model invoke sets session.id on span."""
        ...
        with patch.dict(
            ...
            chat_model = MockBaseChatModel()
            chat_model.invoke("test", session_id="chat-session-789")
